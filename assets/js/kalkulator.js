document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderCart() {
        ...
        itemsEl.innerHTML = stanje.usluge_lista.map(function (item, i) {
            var ps  = item.podusluga;
            var kol = ps.tip_cene !== 'po_dogovoru'
                ? item.kolicina + ' ' + (ps.jedinica_label || 'm²')
                : 'Po dogovoru';
            return '<div class="bk-cart-item">'
                + '<div class="bk-cart-item-info">'
                + '<div class="bk-cart-item-naziv">' + escHtml(item.primarna_naziv + ' › ' + ps.naziv) + '</div>'
                + '<div class="bk-cart-item-kol">' + escHtml(kol) + '</div>'
                + '</div>'
                + '<button class="bk-cart-remove" data-index="' + i + '" aria-label="Ukloni">&#x2715;</button>'
                + '</div>';
        }).join('');

        itemsEl.querySelectorAll('.bk-cart-remove').forEach(function (btn) {
            btn.addEventListener('click', function () {
                stanje.usluge_lista.splice(parseInt(this.getAttribute('data-index')), 1);
                renderCart();
                updateNext1();
            });
        });
    ...
    function buildReview() {
        ...
        stanje.usluge_lista.forEach(function (item, i) {
            var ps = item.podusluga;
            html += '<div class="bk-review-row">'
                + '<span class="bk-review-label">' + (multi ? 'Usluga ' + (i + 1) : 'Usluga') + '</span>'
                + '<span class="bk-review-value">' + escHtml(item.primarna_naziv + ' › ' + ps.naziv) + '</span>'
                + '</div>';
            if (ps.tip_cene !== 'po_dogovoru') {
                html += '<div class="bk-review-row">'
                    + '<span class="bk-review-label">Količina</span>'
                    + '<span class="bk-review-value">' + escHtml(item.kolicina + ' ' + (ps.jedinica_label || 'm²')) + '</span>'
                    + '</div>';
            }
        });

        html += '<div class="bk-review-row"><span class="bk-review-label">Opština</span><span class="bk-review-value">' + escHtml(stanje.opstina_naziv || '—') + '</span></div>';
        html += '<div class="bk-review-row"><span class="bk-review-label">Ugovor</span><span class="bk-review-value">'  + escHtml(stanje.ugovor_naziv)         + '</span></div>';
        html += '<div class="bk-review-row"><span class="bk-review-label">Hitnost</span><span class="bk-review-value">' + escHtml(stanje.hitnost_naziv)        + '</span></div>';
    ...
    function izracunajFinalnu() {
        ...
        var cena_str = (tip === 'raspon' || tip === 'dogovor')
    ...
    btnIzracunaj.addEventListener('click', function () {
        ...
        document.getElementById('result-subtitle').textContent =
            stanje.usluge_lista.length === 1
                ? stanje.usluge_lista[0].primarna_naziv + ' › ' + stanje.usluge_lista[0].podusluga.naziv
        ...
        fin.stavke.forEach(function (s) {
            bdHTML += '<div class="bk-breakdown-item">'
                + '<span class="name">' + escHtml(s.naziv) + '</span>'
                + '<span class="meta">' + (s.kolicina ? escHtml(s.kolicina) : '') + '</span>'
                + '<span class="price">' + escHtml(s.cena_str) + '</span>'
                + '</div>';
        });

        if (stanje.opstina_transport > 0) {
            bdHTML += '<div class="bk-breakdown-item">'
                + '<span class="name">Putni trošak (' + escHtml(stanje.opstina_naziv) + ')</span>'
                + '<span class="meta">+' + fmtRsd(stanje.opstina_transport) + ' RSD</span>'
                + '<span class="price"></span></div>';
        }
        if (stanje.ugovor_procenat !== 0) {
            bdHTML += '<div class="bk-breakdown-item">'
                + '<span class="name">' + escHtml(stanje.ugovor_naziv) + '</span>'
                + '<span class="meta">' + stanje.ugovor_procenat + '%</span>'
                + '<span class="price"></span></div>';
        }
        if (stanje.hitnost_procenat !== 0) {
            bdHTML += '<div class="bk-breakdown-item">'
                + '<span class="name">' + escHtml(stanje.hitnost_naziv) + '</span>'
                + '<span class="meta">+' + stanje.hitnost_procenat + '%</span>'
                + '<span class="price"></span></div>';
        }
        var minTotal = BK_MIN_CENA + stanje.opstina_transport;
        if (fin.cena_rsd <= minTotal || fin.tip === 'dogovor') {
            bdHTML += '<div class="bk-breakdown-item bk-bd-min">'
                + '<span class="name">⚠️ Minimalna cena</span>'
                + '<span class="meta"></span>'
                + '<span class="price">' + fmtRsd(minTotal) + ' RSD</span></div>';
        }
        bd.innerHTML = bdHTML;
        ...
        var stavkeStr    = fin.stavke.map(function (s) {
            return s.naziv + (s.kolicina ? ' (' + s.kolicina + ')' : '') + ': ' + s.cena_str;
        }).join('\n');
        var uslugeNazivi = stanje.usluge_lista.map(function (item) {
